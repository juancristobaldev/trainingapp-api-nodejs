...
module.exports = {
  ...
  userSignInRRSS: async (args, req) => {
    ...
    const { email, profile_image } = args.input;
    ...
    return {
      ...
      user: userResponseEmail
        ? JSON.stringify(userResponseEmail)
        : result
        ? JSON.stringify({ ...dataUser, photo_profile: profile_image })
        : false,
      token: userResponseEmail
        ? userResponseEmail.token
        : result
        ? dataUser.token
        : false,
